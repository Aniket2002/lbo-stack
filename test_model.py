...
if "Error" in results:
    ...
    print(f"Initial Equity Investment: €{equity_investment:,.0f}m")
    ...
    if year_1_data:
        ...
        icr_ratio = (
            year_1_data.get("ebitda", 0) / year_1_data.get("interest_expense", 1)
            if year_1_data.get("interest_expense", 0) > 0
            else float("inf")
        )
        print("\n=== YEAR 1 COVENANTS ===")
        print(f"LTV Ratio: {ltv_ratio:.1f}x (Covenant: {assumptions.leverage_hurdle}x)")
        print(f"ICR Ratio: {icr_ratio:.1f}x (Covenant: {assumptions.icr_hurdle}x)")

        if ltv_ratio > assumptions.leverage_hurdle:
